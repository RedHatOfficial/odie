...
The JumpHost should be given a single disc at least 150GB, 4 CPU and 4096 GB of RAM.  The container ingestion process is a very intensive and relies upon a single threaded docker.  The installation speed is directly tied to your I/O speed.

The ISO itself is bootable.  Set this as the boot media and you will see the following option:

![Custom Grub boot menu](documentation/assets/images/readme-bootmenu.png)

The install may seem to take a while on the *Running post-installation scripts* section.  This is because the contents of the disc are being copied into `/opt/odie/install_media`.

![Long running post-installation scripts](documentation/assets/images/readme-installation-inprogress.png)

At login, the logins are *root*/*odie* and *admin*/*odie*.  Login as root and you will be forced to change the password:

![Mandatory password reset](documentation/assets/images/readme-installation-initial-password.png)


After login, the `odie setup` is already being run in the background.  You can use `systemctl status odie-setup` or `systemctl is-active odie-setup` more directly.  To directly follow the output you can `tail -f /root/odie-setup.log`.  As shown here:

![Mandatory password reset](documentation/assets/images/readme-installation-runsetup.png)

Eventually, the images will be pushed into your registry and you will be ready for installation!

# VM Provision

# OCP Installation

After the JumpHost has been properly configured you may proceed

* Advanced Installation - A standard OCP installation using custom inventory file
* ODIE Installation - for basic installations, this method auto-generes OCP inventory based options defined in custom config files.  Limited use cases supported.

_*NOTE: The ODIE installation was designed for OCP 3.6 and has numerous limitations.  If  *_

## Advanced Installation

At this point, you can proceed with an advanced installation of OpenShift.  For experienced administrators, you can development your own
https://access.redhat.com/documentation/en-us/openshift_container_platform/3.11/html/installing_clusters/install-config-example-inventories[inventory file] and adding the snippet from below (updating hostname to match your env, of course).

```
openshift_web_console_prefix='jumphost01.lab.odie:5000/openshift3/ose-'
oreg_url=jumphost01.lab.odie:5000/openshift3/ose-${component}:${version}
openshift_docker_additional_registries=jumphost01.lab.odie:5000
openshift_docker_insecure_registries=jumphost01.lab.odie:5000
openshift_logging_image_prefix=jumphost01.lab.odie:5000/openshift3/
openshift_logging_image_version=v3.11
openshift_metrics_image_prefix=jumphost01.lab.odie:5000/openshift3/
...
* These are all being evaluated in a pure ODIE context, expect some additional context in this group. *

## ODIE Functions


<< need odie.sh --help dump here >>

### `odie setup`

As documented in the example above, this aggregate function combines the commands `odie properties`, `odie configure`, and `odie push` for a push-button ODIE setup process.

Additionally, if your gateway hosts a conifugration archive `odie-config.tar.gz` accessible via at the root of your gateway server (e.g. http://192.168.124.1/odie-config.tar.xz) that will be downloaded as part of `odie setup`.  If the *auto_start* variable is set in `odie.yml` then an automatic installation is triggered.
...
These can also be done as independent operations `odie generate-kickstart` and `odie generate-ocp-inventory` respectively.
...
This is the command that actually installs OCP.  Your VMs must have alrady been configured.  
...
## My build failed and now the permissions are messed up!
